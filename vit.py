...
class VisionTransformer(nn.Module):
    def __init__(
            ...
            top_k=None, batchwise_prompt=False, prompt_key_init='uniform', head_type='token', use_prompt_mask=False,ways=None,num_tasks=None):
        super().__init__()
        ...
        if prompt_length is not None and pool_size is not None and prompt_pool:
            self.prompt = Prompt(length=prompt_length, embed_dim=embed_dim, embedding_key=embedding_key, prompt_init=prompt_init,
                    ...
                    prompt_key_init=prompt_key_init,num_classes=num_classes,ways=ways,num_tasks=num_tasks)
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False,prompts_matrix=None,):
        ...
        if hasattr(self, 'prompt'):
            if self.use_prompt_mask and train:
                ...
                prompt_mask = None
            res = self.prompt(x, prompt_mask=prompt_mask, cls_features=cls_features,prompts_matrix=prompts_matrix,is_training=train,task_id=task_id)
    ...
    def forward(self, x, task_id=-1, cls_features=None, train=False,prompts_matrix=None):
        res = self.forward_features(x, task_id=task_id, cls_features=cls_features, train=train,prompts_matrix=prompts_matrix)
